...
EFFECT.MainMat = Material("vj_cofr/beam")
...
function EFFECT:Init(data)
    ...
    util.Decal("VJ_COFR_Scorch_Small", self.EndPos + data:GetNormal(), self.EndPos - data:GetNormal())
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function EFFECT:Render()
    ...
    render.DrawBeam(self.StartPos, self.EndPos, math.Rand(18,24), math.Rand(0,1), math.Rand(0,1) + ((self.StartPos - self.EndPos):Length() / 128), Color(255, 0, 0, 255,(50 / ((self.DieTime - 0.5) - CurTime()))))
end