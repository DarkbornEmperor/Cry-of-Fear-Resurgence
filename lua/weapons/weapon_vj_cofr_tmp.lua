...
SWEP.WorldModelOffsetParams = {
    Enabled = true,
    Bone = "Bip01 R Hand",
    Pos = Vector(2.624, 1.258, 2.79),
    Ang = Angle(-5.977, 5.027, 89.475)
}
-- Primary Fire ---------------------------------------------------------------------------------------------------------------------------------------------
SWEP.Primary.Damage = 7
SWEP.Primary.ClipSize = 30
SWEP.Primary.Ammo = "SMG1"
SWEP.Primary.Sound = "VJ.CoFR_TMP.Loop"
SWEP.Primary.EndSound = "VJ.CoFR_TMP.Single"
...
function SWEP:Init()
    if GetConVar("VJ_COFR_OldWepSounds"):GetInt() == 1 then
        self.Primary.Sound = "VJ.CoFR_TMP_Old.Loop"
        self.Primary.EndSound = "VJ.CoFR_TMP_Old.Single"
    end
    ...
    timer.Simple(0.1, function()
        if IsValid(self) && IsValid(self:GetOwner()) then
            ...
            if ownerClass == "npc_vj_cofr_simon_beta" then
                self.WorldModelOffsetParams.Pos = Vector(2.181, 1.214, 2.24)
            elseif ownerClass == "npc_vj_cofr_police" then
                self.WorldModelOffsetParams.Pos = Vector(2.676, 1.258, 2.292)
            end
        end
    end)
    if self.Primary.UsesLoopedSound then
        ...
        self.PrimaryLoop:SetSoundLevel(140)
    end
        ...
        if CurTime() > self.PrimaryLoopSoundT && self.PrimaryLoopSoundT > 0 then
            ...
            if IsValid(owner) then
                ...
                if fireSd != false then
                    sound.Play(fireSd, owner:GetPos(), 140, math_random(self.Primary.SoundPitch.a, self.Primary.SoundPitch.b), self.Primary.SoundVolume)
                end
            end
